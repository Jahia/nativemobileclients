...
window.addEventListener('load', function(){
    cordovadesktop.internal.initialiseData();
    
    setTimeout(function(){
    
        // Map mouse events to touch events
        document.onmousedown = function(e){
            cordovadesktop.internal.dispatchTouchEvent(e, "touchstart");
            cordovadesktop.internal.touchActive = true;
        };
        
        document.onmousemove = function(e){
            if (cordovadesktop.internal.touchActive) {
                cordovadesktop.internal.dispatchTouchEvent(e, "touchmove");
            }
        };
        
        document.onmouseup = function(e){
            cordovadesktop.internal.touchActive = false;
            cordovadesktop.internal.dispatchTouchEvent(e, "touchend");
        };
        ...
        document.onkeydown = function(e){
            ...
            if (e.altKey && e.ctrlKey) {
                cordovadesktop.internal.fireEvent(e.keyCode);
            }
        };
        
        cordovadesktop.utility.timedPopup(25, 90, 70, 7, "PhoneGap Desktop<br/>Ctrl+Alt + H for events help", 1000, "DarkBlue");
        
        document.removeEventListener("DOMContentLoaded", arguments.callee, false);
        var readyEvent = document.createEvent('HTMLEvents');
        readyEvent.initEvent('deviceready', true, true);
        document.dispatchEvent(readyEvent);
    }, cordovadesktop.internal.getDebugValue("internal", "startupDelay") || 200)
}, false);


// Internal library functions 
var cordovadesktop = {};
cordovadesktop.internal = {
    debugdata: {},
    
    initialiseData: function(){
        if (cordovaDesktopDebugData) {
            cordovadesktop.internal.debugdata = cordovaDesktopDebugData;
            return;
        }

        // Load the default config file
        this.parseConfigFile('cordovadesktop/debugdata.js');
    ...
    parseConfigFile: function(fileName){
        ...
        jsonReq.send();
        	...
        	cordovadesktop.utility.timedPopup(10, 10, 80, 20, "Cordova Desktop<br/>Unable to load JSON data. Check setup<br/>" + err, 3000, "DarkRed")
        }
        
        data = jsonReq.responseText;
        console.log("data=" + data);
        
        cordovadesktop.internal.debugdata = cordovadesktop.utility.mergeRecursive(cordovadesktop.internal.debugdata, JSON.parse(data));
    },
    ...
    getDebugValue: function(nodeName, element){
        ...
        if (Object.prototype.toString.call(node[element]) === '[object Array]') {
            ...
            if (node.arraySequence != undefined) {
                ...
                inSequence = cordovadesktop.internal.debugdata.internal.arraySequence;
            }
            if (inSequence) {
    ...
    fireEvent: function(keyCode){
        ...
        switch (keyCode) {
            case 72: // <H>elp
                cordovadesktop.utility.timedPopup(10, 10, 77, 78, helpHTML, 3000, "DarkBlue");
                break;
            case 80: // <P>ause
                eventName = 'pause';
                break;
            ...
            case 73: // Battery Cr<i>tical
                ...
                cordovadesktop.internal.setBatteryProperties(customEvent);
                windowEvent = true;
                break;
    ...
    setBatteryProperties: function(eventObject){
        var batteryInfo = cordovadesktop.internal.getDebugValue('events', 'battery');
        cordovadesktop.utility.mergeRecursive(eventObject, batteryInfo);
    },
    ...
    setDynamicProperty: function(obj, debugElement, debugNode){
        ...
        if (obj.__defineGetter__) {
            obj.__defineGetter__(debugNode, function(){
                return cordovadesktop.internal.getDebugValue(debugElement, debugNode);
            });
        }
    ...
    intervalFunction: function(successCallback, errorCallback, frequency, nodeName, elementName, errorElement){
        return setInterval(function(){
            if (cordovadesktop.internal.randomException(nodeName)) {
                if (errorElement) {
                    errorCallback(cordovadesktop.internal.getDebugValue(nodeName, errorElement));
                }
                else {
                ...
                successCallback(cordovadesktop.internal.getDebugValue(nodeName, elementName));
            }
        }, frequency);
    },
    ...
    mediaHandler: function(successCallback, errorCallback, options){
        if (cordovadesktop.internal.randomException("capture")) {
            errorCallback(cordovadesktop.internal.getDebugValue('capture', 'error'));
        }
        else {
            var mediaFiles = cordovadesktop.internal.getDebugValue('capture', 'media');
            for (var media in mediaFiles) {
                media = new cordovadesktop.objects.MediaFile(media);
            }
    ...
    beeper: function(times){
        if (times > 0) {
            cordovadesktop.utility.timedPopup(20, 90, 60, 5, "Beep", 300, "Green", cordovadesktop.internal.beepPause, times - 1);
        }
    },
    ...
    beepPause: function(times){
        if (times > 0) {
            setTimeout(function(){
                cordovadesktop.internal.beeper(times)
            }, 300);
        }
    }
        ...
        for (var p in obj2) {
            try {
                ...
                if (obj2[p].constructor === Object) {
                    obj1[p] = cordovadesktop.utility.mergeRecursive(obj1[p], obj2[p]);
                }
                else {
                    obj1[p] = obj2[p];
                }
        ...
        if (obj instanceof Object) {
            ...
            for (var attr in obj) {
                if (obj.hasOwnProperty(attr)) 
                    copy[attr] = cordovadesktop.utility.clone(obj[attr]);
            }
            return copy;
        }
...
cordovadesktop.objects = {

    MediaFile: function(data){
        // Copy the properties into this object
        cordovadesktop.utility.mergeRecursive(this, data);
    }
...
navigator.accelerometer = {
    getCurrentAcceleration: function(accelerometerSuccess, accelerometerError){
        if (cordovadesktop.internal.randomException("accelerometer")) {
            accelerometerError('A randome error was generated');
        }
        else {
            accelerometerSuccess(cordovadesktop.internal.getDebugValue('accelerometer', 'acceleration'));
        }
    },
    watchAcceleration: function(accelerometerSuccess, accelerometerError, accelerometerOptions){
        return cordovadesktop.internal.intervalFunction(accelerometerSuccess, accelerometerError, accelerometerOptions.frequency, "accelerometer", "acceleration");
    },
    clearWatch: function(watchID){
        cordovadesktop.internal.cancelIntervalFunction(watchID);
    }
};
...
navigator.camera = {
    getPicture: function(cameraSuccess, cameraError, cameraOptions){
        if (cordovadesktop.internal.randomException("camera")) {
            ...
            if (cameraOptions && cameraOptions.destinationType !== undefined && cameraOptions.destinationType === Camera.DestinationType.FILE_URI) {
                cameraSuccess(cordovadesktop.internal.getDebugValue('camera', 'pictures_url'));
            }
            else {
                if (cameraOptions.encodingType === Camera.EncodingType.PNG) {
                    cameraSuccess(cordovadesktop.internal.getDebugValue('camera', 'pictures_base64_png'));
                }
                else {
                    cameraSuccess(cordovadesktop.internal.getDebugValue('camera', 'pictures_base64_jpg'));
                }
            }
        }
    }
...
navigator.device.capture = {
    captureAudio: function(captureSuccess, captureError, options){
        cordovadesktop.internal.mediaHandler(captureSuccess, captureError, options);
    },
...
cordovadesktop.objects.MediaFile.prototype.getFormatData = function(successCallback, errorCallback){
    if (cordovadesktop.internal.randomException("capture")) {
        errorCallback();
    }
    else {
        successCallback(cordovadesktop.internal.getDebugValue('capture', 'mediaformat'));
    }
};
...
navigator.compass = {
    getCurrentHeading: function(compassSuccess, compassError, compassOptions){
        if (cordovadesktop.internal.randomException("compass")) {
            compassError(cordovadesktop.internal.getDebugValue('compass', 'error'));
        }
        else {
            compassSuccess(cordovadesktop.internal.getDebugValue('compass', 'heading'));
        }
    },
    watchHeading: function(compassSuccess, compassError, compassOptions){
        return cordovadesktop.internal.intervalFunction(compassSuccess, compassError, compassOptions.frequency, "compass", "heading", "error");
    },
    clearWatch: function(watchID){
        cordovadesktop.internal.cancelIntervalFunction(watchID);
    },
    watchHeadingFilter: function(compassSuccess, compassError, compassOptions){
        return cordovadesktop.internal.intervalFunction(compassSuccess, compassError, compassOptions.frequency || 5000, "compass", "heading", "error");
    },
...
cordovadesktop.internal.setDynamicProperty(navigator.network.connection, "connection", "type");
...
navigator.contacts = {
    create: function(properties){
        ...
        cordovadesktop.utility.mergeRecursive(tempContact, properties);
        return tempContact;
    },
    find: function(contactFields, contactSuccess, contactError, contactFindOptions){
        if (cordovadesktop.internal.randomException("contacts")) {
            contactError(cordovadesktop.internal.getDebugValue("contacts", "error"));
        }
        else {
            contactSuccess(cordovadesktop.internal.getDebugValue("contacts", "array"));
        }
    }
};
...
function Contact(id, displayName, name, nickname, phoneNumbers, emails, addresses, ims, organizations, birthday, note, photos, categories, urls){
    ...
    this.clone = function(){
        return cordovadesktop.utility.clone(that);
    };
    
    this.save = function(saveSuccess, saveError){
        if (cordovadesktop.internal.randomException("contacts")) {
            saveError(cordovadesktop.internal.getDebugValue("contacts", "error"));
        }
        else {
            saveSuccess(that);
        }
...
Contact.prototype.remove = function(removeSuccess, removeError){
    if (cordovadesktop.internal.randomException("contacts")) {
        removeError(cordovadesktop.internal.getDebugValue("contacts", "error"));
    }
    else {
        removeSuccess(cordovadesktop.internal.getDebugValue("contacts", "array"));
    }
...
window.device = {};
cordovadesktop.internal.setDynamicProperty(window.device, "device", "name");
cordovadesktop.internal.setDynamicProperty(window.device, "device", "cordova");
cordovadesktop.internal.setDynamicProperty(window.device, "device", "platform");
cordovadesktop.internal.setDynamicProperty(window.device, "device", "uuid");
cordovadesktop.internal.setDynamicProperty(window.device, "device", "version");
...
window.requestFileSystem(LocalFileSystem.TEMPORARY, 1, function(fs){
    console.log("Successfully acquired temporary filesystem " + fs);
    resolveLocalFileSystemURI.prototype.tempFS = fs;
});

window.requestFileSystem(LocalFileSystem.PERSISTENT, 1, function(fs){
    console.log("Successfully acquired persistent filesystem " + fs);
...
navigator.geolocation.getCurrentPosition = function(geolocationSuccess, geolocationError, geolocationOptions){
    if (cordovadesktop.internal.randomException("geolocation")) {
        geolocationError(cordovadesktop.internal.getDebugValue("geolocation", "error"));
    }
    else {
        geolocationSuccess(cordovadesktop.internal.getDebugValue("geolocation", "position"));
    }
};

navigator.geolocation.watchPosition = function(geolocationSuccess, geolocationError, geolocationOptions){
    return cordovadesktop.internal.intervalFunction(geolocationSuccess, geolocationError, geolocationOptions.frequency, "geolocation", "position", "error");
};
...
function Media(src, mediaSuccess, mediaError, mediaStatus){
    ...
    this.play = function(){
        if (cordovadesktop.internal.randomException("media")) {
            that.ErrorCallback(cordovadesktop.internal.getDebugValue("media", "error"));
        }
        else {
			if (supportedFile()) {
				audioObj.play();
		        ...
		        cordovadesktop.utility.timedPopup(35, 90, 60, 5, "Unsupported audio: " + (src.substr(src.lastIndexOf('.')) || src), 1000, "DarkBlue");
			}
        }
    };
    this.pause = function(){
...
Media.prototype.getCurrentPosition = function(mediaSuccess, mediaError){
    if (cordovadesktop.internal.randomException("media")) {
        mediaError(cordovadesktop.internal.getDebugValue("media", "error"));
    }
    else {
        mediaSuccess(cordovadesktop.internal.getDebugValue("media", "position"));
    }
};

Media.prototype.getDuration = function(){
    return cordovadesktop.internal.getDebugValue("media", "duration");
};
...
navigator.notification = {
    ...
    vibrate: function(milliseconds){
        cordovadesktop.utility.timedPopup(20, 90, 60, 5, "Vibrate", milliseconds, "Orange");
    }
};
